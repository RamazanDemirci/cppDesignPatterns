...
class OldIRCameraAdapter : public Camera
{
private:
    OldIRCamera oldIRCamera;

public:
    virtual double getZoomValue() override { oldIRCamera.getZoomDegree(); };
    virtual void zoomIn() override { oldIRCamera.applyZoomIn(); };
    virtual void zoomOut() override { oldIRCamera.applyZoomOut(); };
    virtual void zoomDirect(double value) override { oldIRCamera.setZoom(value); };
    virtual VideoFormat getVideoFormat() override { oldIRCamera.getVideoCompression(); };
    virtual void setVideoFormat(VideoFormat format) override
    {
        switch (format)
        {
        case VideoFormat::H264:
            oldIRCamera.setVideoCompression(VideoCompression::IR_H264);
        case VideoFormat::MJPEG:
            oldIRCamera.setVideoCompression(VideoCompression::IR_MJPEG);
        case VideoFormat::MPEG4:
            oldIRCamera.setVideoCompression(VideoCompression::IR_MPEG4);
        default:
